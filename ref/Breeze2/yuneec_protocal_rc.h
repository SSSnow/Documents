...
typedef enum {
	AUTO_NONES          = 0,	// idle state
	AUTO_JOUR  			= 1,	// Journey mode
	AUTO_POI  			= 2,	// Point Of Interest mode
	AUTO_CURVECABLECAM	= 3,	// CCC mode
	AUTO_ORBIT 			= 4,	// Orbit mode
	AUTO_WAYPOINT 		= 5,	// reserved for wizard point to fly feature
	AUTO_MAP_WAYPOINT 	= 6,	// for gcs waypoint from map
	MAP_FENCE 			= 7,	// for fence from map
} ModeType_e;

typedef enum {
	ACTION_REQUEST_START_AUTO    = 0,	// start auto/task mode
	ACTION_REQUEST_START_MANUAL  = 1,	// start manual control mode
	ACTION_REQUEST_PAUSE         = 2,	// pause task mode
	ACTION_REQUEST_RESUME        = 3,	// resume task mode
	ACTION_REQUEST_EXIT          = 4,	// exit task mode
	ACTION_REQUEST_GET           = 5,	// get the corresponding configuration back to GCS
	ACTION_REQUEST_SET           = 6,	// sending wp data to MAV, in a list way
	ACTION_REQUEST_CHECK		 = 7, 	// ask to return wp to GCS for check
	ACTION_REQUEST_CONFIG        = 8,	// configure waypoint
} ActionRequest_e;

typedef enum {
	ACTION_REQUEST_START_PHOTO   	= 0,
	ACTION_REQUEST_START_SHOOTING	= 1,
} CameraActionRequest_e;

typedef enum {
	ACTION_RESULT_OK = 0,					// good result
	...
	ACTION_RESULT_ERR_ACCESS_OUTRANGE, 		// invalid target position or height
	ACTION_RESULT_ERR_COORDINATE_TOOCLOSE, 	// aircraft is too close
	ACTION_RESULT_ERR_COORDINATE_DANGER, 	// dangerous path
	ACTION_RESULT_ERR_NO_GIMBAL,			// no gimbal mounted
	ACTION_RESULT_ERR_NOFLYZONE,			// enter no-fly-zone
	ACTION_RESULT_ERR_LOWBATTERY,			// low battery while active task mode
} ActionResult_e;
...
typedef struct {
	...
	uint8_t  actionRequest;	    // from enum ActionResult_e
...
typedef struct {
	uint8_t  actionType;		// from enum ActionType_e, the first byte is used to identify ActionType_e
	uint8_t  actionResult;		// from enum ActionResult_e
...
typedef struct {
	...
	int32_t  lon;			// longitude (degrees)	+/- 180 deg
	int32_t  alt;			// 0.01m resolution, altitude (meters)
	uint8_t  radius;		// radius from point of interest to the vehicle
...
typedef struct {
	uint8_t actionType;		// from enum ActionType_e, the first byte is used to identify ActionType_e
	uint8_t actionResult;	// from enum ActionResult_e
	uint8_t maxSpeed;       // maximum speed: 2-10m/s default:7m
	uint8_t maxDistance;	// maximum distance: 20-100m default: 30m
} ActionSettingJOUR;

typedef struct {
	uint8_t  actionType;			// from enum ActionType_e, the first byte is used to identify ActionType_e
	uint8_t  actionState;			// from enum ActionState_e
	uint8_t  modeType;				// from enum ModeType_e
	uint8_t  degreeOfCompletion;	// degree of completion of waypoint/CCC mode
} ActionFeedback;

typedef struct {
	uint8_t actionType;		// from enum ActionType_e, the first byte is used to identify ActionType_e
	uint8_t takeoffSpeed;	// vertical speed in cm/s for one key takeoff
	uint8_t takeoffHeight;	// height of one key takeoff in cm
} ActionTakeOff;

typedef struct {
	uint8_t  actionType;	// from enum ActionType_e, the first byte is used to identify ActionType_e
	uint8_t  activeCA;		// 0 for off, 1 for on
} ActionCAConfig;
...
typedef struct {
	uint8_t actionType;		// from enum ActionType_e, the first byte is used to identify ActionType_e
	uint8_t whichConfig;	// send back which configuration, this value should be the actionType for this configuration.
} ActionAskForConfig;
...
typedef struct {
	...
	CameraActionRequest_e actionRequest;
...
$$ get point:
	...
	2.	ACTION_REQUEST(ACTION_REQUEST_GET) 		==>
							 
							    				<==			ACTIONSETTING_CCC//only send setting data when ACTION_RESPONSE is ok
	
	3.	ACTION_REQUEST(ACTION_REQUEST_GET) 		==>
												...
												.
												.
	n.	ACTION_REQUEST(ACTION_REQUEST_GET) 		==>
